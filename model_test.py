...
df = pd.read_csv('fraud_sample_new2.csv')
cols_dt  = {}
cols = []
...
lda = LinearDiscriminantAnalysis(n_components=120, priors=None, shrinkage=None,
...
X_test = lda.fit_transform(X_test,y=y_test)
...
for i in range(350):
    ...
    #clf = BaggingClassifier(KNeighborsClassifier(n_neighbors=n_neighbors), max_samples=0.5, max_features=0.5,n_estimators=trees_2)
    #clf = AdaBoostClassifier(n_estimators=trees_1, random_state=0)
    #clf = RandomForestClassifier(n_estimators=trees_1, max_depth=depth_1,random_state=0,class_weight = 'balanced')
    clf = BaggingClassifier(DecisionTreeClassifier(max_depth=depth_1),max_samples = 0.75, max_features = 120,n_estimators=trees_2)
    #clf = BaggingClassifier(SVC(kernel="rbf",random_state=0),max_samples = 0.5, max_features = 0.5,n_estimators=trees_2)
    #clf = SVC(kernel="rbf",random_state=0)
    clas = classify(120,X_train,y_train,X_test,y_test,clf,max_f1)
    clas.fit()
    clas.predict()
